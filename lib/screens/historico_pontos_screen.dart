...
class HistoricoPontosScreen extends StatefulWidget {
  @override
  _HistoricoPontosScreenState createState() => _HistoricoPontosScreenState();
}

class _HistoricoPontosScreenState extends State<HistoricoPontosScreen> {
  ...
  Future<void> _loadUserData() async {
    ...
    final idUsuarioStr = prefs.getString('id_usu');
    if (idUsuarioStr != null) {
      setState(() {
        _idUsuario = int.parse(idUsuarioStr);
      });
    }
  }
  ...
  Future<void> _loadPontos() async {
    ...
    try {
      final pontos = await _attendanceService.getPontosUsuario(_idUsuario!);
      setState(() {
        _pontos = pontos;
        _isLoading = false;
      });
  ...
  Future<void> _loadPontosByDate(String data) async {
    ...
    try {
      final pontos = await _attendanceService.getPontosByDay(_idUsuario!, data);
      setState(() {
        _pontos = pontos;
        _isLoading = false;
      });
